...
// 직선 객체 임시 생성 반환
function returnTempLine(me, startX, startY){
	var curX = me.offsetX;
	var curY = me.offsetY;
	var tmp = new ShapeObj("Line", new coord(startX, startY), new coord(curX, curY), myContext.strokeStyle, null, -1);
	return tmp;
}
...
function drawLine(LineObj){
	...
	myContext.beginPath();
	myContext.moveTo(LineObj.startpoint.x, LineObj.startpoint.y);
	myContext.lineTo(LineObj.endpoint.x, LineObj.endpoint.y);
	myContext.stroke();
}
...
// 직선의 양끝점, 도형의 경우는 obj를 감싸는 최소 직사각형 프레임 가시화
